...
async function getServersFromDB() {
    try {
        const [rows] = await pool.query('SELECT id, primaryip, phonenumber FROM healthcheck');
        return rows;
    } catch (err) {
...
function monitorServers(SERVERS) {
    ...
    const interval = setInterval(async () => {
        if (activeServers.length === 0) {
            console.log('✅ All servers alerted. Stopping monitoring.');
        ...
        for (let i = activeServers.length - 1; i >= 0; i--) {
            ...
            const { id, primaryip, phonenumber } = server;
            ...
            if (isUp !== wasUp) {
                ...
                console.log(`🛑 ${name} alert sent. Removed from further checks.`);
            }
        }

        console.log('🔁 One check cycle completed.');
    }, CHECK_INTERVAL);
}
